...
# Set time delay between iterations (in seconds)
time_delay = 60
...
while True:
    ...
    if items:
        ...
        for worker_id in ['MW1', 'MW2', 'MW3', 'MW4', 'MW5', 'FO1', 'FO2', 'FO3', 'IM1', 'IM2', 'MT1', 'MT2']:
            ...
            if not worker_df.empty:
                ...
                # Provide more personalized training recommendations based on job role
            if worker_id.startswith('MW'):
                # Training recommendations for Maintenance Workers
                if predicted_stress_level[0] == 1:
                    training_recommendation = "You have low stress levels. Continue with your regular training routine. Consider adding stretching exercises to your routine to improve flexibility and prevent injuries."
                else:
                    training_recommendation = "You have high stress levels, which may be due to the physical demands of your job. We recommend modifying your training routine to reduce stress and prevent injuries. Focus on exercises that improve muscular endurance and strength, and include rest days in your routine to allow for recovery. Consider adding exercises that improve balance and coordination, as these skills are important for performing tasks safely and efficiently."

            elif worker_id.startswith('FO'):
                # Training recommendations for Forklift Operators
                if predicted_stress_level[0] == 1:
                    training_recommendation = "You have low stress levels. Continue with your regular training routine. Consider adding cardio exercises to your routine to improve cardiovascular health."
                else:
                    training_recommendation = "You have high stress levels, which may be due to the prolonged sitting and repetitive motions associated with operating a forklift. We recommend modifying your training routine to reduce stress and prevent injuries. Focus on exercises that improve flexibility and mobility, and include rest days in your routine to allow for recovery. Consider adding exercises that improve core strength and posture, as these skills are important for maintaining a healthy spine and preventing back pain."

            elif worker_id.startswith('IM'):
                # Training recommendations for Inventory Managers
                if predicted_stress_level[0] == 1:
                    training_recommendation = "You have low stress levels. Continue with your regular training routine. Consider adding exercises that improve upper body strength and posture to maintain good health and prevent injuries."
                else:
                    training_recommendation = "You have high stress levels, which may be due to the prolonged standing and repetitive motions associated with inventory management. We recommend modifying your training routine to reduce stress and prevent injuries. Focus on exercises that improve lower body strength and flexibility, and include rest days in your routine to allow for recovery. Consider adding exercises that improve balance and coordination, as these skills are important for maintaining stability and preventing falls."

            elif worker_id.startswith('MT'):
                # Training recommendations for Maintenance Technicians
                if predicted_stress_level[0] == 1:
                    training_recommendation = "You have low stress levels. Continue with your regular training routine. Consider adding exercises that improve flexibility and core strength to maintain good health and prevent injuries."
                else:
                    training_recommendation = "You have high stress levels, which may be due to the physical demands of your job as a maintenance technician. We recommend modifying your training routine to reduce stress and prevent injuries. Focus on exercises that improve muscular endurance and strength, and include rest days in your routine to allow for recovery. Consider adding exercises that improve balance and coordination, as these skills are important for performing tasks safely and efficiently. Additionally, consider adding exercises that improve grip strength, as this is important for performing tasks that require gripping tools and equipment."
    ...
    # Wait for the specified time delay before checking for new data
